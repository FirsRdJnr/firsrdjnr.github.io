...
// for when the cube is clicked a few times
...
// set timeout on info panel for after video has payed
var playInfo = '.prize-cube__play';
var addBlur = function() {
        $('.prize-cube__video').addClass('done');
    }

var tlPlayVid = new TimelineLite()

TweenMax.set(playInfo, {autoAlpha:0});

tlPlayVid.to(playInfo, .4, {autoAlpha:1}, 7);
tlPlayVid.add(addBlur, "-=1");

$('.inToGame').click(function() {
    $('.prize-cube__play').addClass('hide');
    $('.prize-cube__video').addClass('hide');
    tl.play();
})
...
// TweenMax.set(cube, {rotationY:0});
TweenMax.set(reveal, {y: docH,});

// spin cude
// tl.to(cube, 1.4, {rotationY:-720, ease:CustomEase.create("custom", easeBounce)});
// tl.to(cube, 1.4, {rotationX:0}, '-=1.4');
tl.add(panelShow)
tl.to(reveal, .4, {y: 0, ease:CustomEase.create("custom", easeBounce)}, 1.5);
